...
Parser.prototype.parse = function(rules, source, imports) {
  ...
  return rules.reduce(function(acc, rule) {
    ...
    if (select(rule, ':constructor')) return self.visit_constructor(rule, source, acc);
...
Parser.prototype.visit_constructor = function(rule, source, rules) {
  rule.declarations.forEach(function(dec) {
    var cons = JSON.parse(dec.value);
    // TODO catch error
    // TODO pass filename
    var ast = jade2ast(cons);
    rules.push(ast[0]);
  });
  return rules;
};
...
Parser.prototype.visit_rule = function(rule, source, rules) {
  ...
  var name = '[' + rule.selectors.map(function(sel) {
    return '"' + sel.replace(inParens, function(_, p1, p2, offset, string) {
      if (p1 &&  p1 === ':nth-child') return p1 + '(' + p2 + ')';
      return '" + ' + p2 + ' + "';
    }) + '"';
  }).join(',') + ']';
